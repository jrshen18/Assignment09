...
Jeffrey Shen, 2020-Mar-20, added to save_inventory function
...
class FileIO:
    ...
    def save_inventory(file_name: list, lst_Inventory: list) -> None:
        ...
        try:
            ...
            with open(file_name_track, 'w') as f:
                for item in lst_Inventory:
                    data = '{}, {}'.format(item.cd_id, item.get_track_info())
                    f.write(data)
    ...
    def load_inventory(file_name: list) -> list:
        ...
        try:
            ...
            with open(file_name_track, 'r') as f:
                for line in f:
                   data = line.strip().split(',')
                   cd = DC.Track(data[0], data[1], data[2])
                   cd.add_track(cd)
        except Exception as e:
            print('There was a general error!', e, e.__doc__, type(e), sep='\n')
